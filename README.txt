...
- Chicago is an analysis pipeline for varant format call (VCF) file that:
	1. Retrieves multi-trasncript Ensembl effects for each variant
...
- Chicago is a bash script desinged to be run from the command line.  It requres the user to Java installed, as well as Perl and Python in the approprate paths.  
- Note: The first run of main.sh will install will install Reference_resources directory which contains resources for lift overs to hg38 and indel normalizatipn
